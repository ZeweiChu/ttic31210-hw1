...
def eval(model, data, args, pos_data=None, att_dict={}):
	...
	for idx, (mb_d, mb_mask_d, mb_l, mb_pos) in enumerate(data):
		# print(mb_d)
		mb_d = Variable(torch.from_numpy(mb_d)).long()
		# print("after")
		# print(mb_d)
		mb_mask_d = Variable(torch.from_numpy(mb_mask_d))
		mb_pos = Variable(torch.from_numpy(mb_pos)).long()
		mb_out = model(mb_d, mb_mask_d, pos=mb_pos, att_dict=att_dict)
		# print(mb_out)
...
def main(args):
	...
	docs, labels, pos_docs = utils.encode(train_examples, word_dict, pos_examples=train_pos_examples, pos_dict=pos_tag_dict)
	all_train = utils.gen_examples(docs, labels, args.batch_size, pos=pos_docs)
	
	d_docs, d_labels, pos_d_docs = utils.encode(dev_examples, word_dict, pos_examples=dev_pos_examples, pos_dict=pos_tag_dict)
	total_dev = len(d_docs)
	all_dev = utils.gen_examples(d_docs, d_labels, args.batch_size, pos=pos_d_docs)
